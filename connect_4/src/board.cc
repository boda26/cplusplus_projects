#include "board.hpp"

// you might need additional includes
#include <iomanip>
#include <iostream>
#include <sstream>
#include <string>

/**************************************************************************/
/* your function definitions                                              */
/**************************************************************************/

void InitBoard(Board& b) {
  // below was minimal to get tests to actually compile and given test case to
  // fail
  for (int i = 0; i < Board::kBoardHeight; ++i) {
    for (int j = 0; j < Board::kBoardWidth; ++j) {
      b.board[i][j] = DiskType::kEmpty;
    }
  }
}


void DropDiskToBoard(Board& b, DiskType disk, int col) {
  if (col < 0 || col >= Board::kBoardWidth) {
    throw std::runtime_error("Invalid column!");
    return;
  }
  for (int i = 0; i < Board::kBoardHeight; ++i) {
    if (b.board[i][col] == DiskType::kEmpty) {
      b.board[i][col] = disk;
      return;
    }
  }
  throw std::runtime_error("The column is full!");
}


bool CheckForWinner(Board& b, DiskType disk) {
  bool h = SearchForWinner(b, disk, WinningDirection::kHorizontal);
  bool v = SearchForWinner(b, disk, WinningDirection::kVertical);
  bool l = SearchForWinner(b, disk, WinningDirection::kLeftDiag);
  bool r = SearchForWinner(b, disk, WinningDirection::kRightDiag);
  return (h || v || l || r);
}


bool SearchForWinner(Board& b, DiskType disk, WinningDirection to_check) {
  bool hasfour = false;
  if (to_check == WinningDirection::kHorizontal) {
    for (int i = 0; i < Board::kBoardHeight; ++i) {
      int count = 0;
      for (int j = 0; j < Board::kBoardWidth; ++j) {
        if (b.board[i][j] == disk) {count++; if (count >= 4) {hasfour = true;}} else {count = 0;}}}
  } else if (to_check == WinningDirection::kVertical) {
    for (int i = 0; i < Board::kBoardWidth; i++) {
      int count = 0;
      for (int j = 0; j < Board::kBoardHeight; j++) {
        if (b.board[j][i] == disk) {
          count++;
          if (count >= 4) {
            hasfour = true;
          }
        } else {
          count = 0;
        }
      }
    }
  } else if (to_check == WinningDirection::kLeftDiag) {
    int maxsum = Board::kBoardHeight + Board::kBoardWidth - 2;
    for (int sum = 0; sum <= maxsum; ++sum) {
      int count = 0;
      for (int i = 0; i < Board::kBoardHeight; ++i) {
        for (int j = 0; j < Board::kBoardWidth; ++j) {
          if (i + j - sum == 0) {
            if (b.board[i][j] == disk) {
              count++;
              if (count >= 4) {
                hasfour = true;
              } 
            } else {
              count = 0;
            }
          }
        }
      }
    }
  } else if (to_check == WinningDirection::kRightDiag) {
    int max = std::max(Board::kBoardHeight, Board::kBoardWidth);
    int min = std::max(0 - Board::kBoardHeight, 0 - Board::kBoardWidth);
    for (int diff = max; diff >= min; diff--) {
      int count = 0;
      for (int i = 0; i < Board::kBoardHeight; i++) {
        for (int j = 0; j < Board::kBoardWidth; j++) {
          if (i - j == diff) {
            if (b.board[i][j] == disk) {
              count++;
              if (count >= 4) {hasfour = true;}} else {count = 0;}
          }
        }
      }
    }
  }
  return hasfour;
}


bool BoardLocationInBounds(int row, int col) {
  bool b = false;
  if (row >= 0 && row < Board::kBoardHeight && col >= 0 && col < Board::kBoardWidth) {
    b = true;
  }
  return b;
}


/**************************************************************************/
/* provided to you                                                        */
/**************************************************************************/
std::string BoardToStr(const Board& b) {
  constexpr int kTotalWidth = Board::kBoardWidth * 11 - Board::kBoardHeight - 1;
  std::stringstream ss;
  ss << std::endl;
  for (int row = Board::kBoardHeight; row > 0; --row) {
    ss << " |";
    for (int col = 0; col < Board::kBoardWidth; ++col) {
      std::string value_here;
      value_here.push_back(static_cast<char>(b.board[row - 1][col]));
      ss << ' ' << CenterStr(value_here, Board::kBoardWidth + 1) << '|';
    }
    ss << std::endl;
    ss << " |" << std::setw(kTotalWidth) << std::setfill('-') << '|'
       << std::endl;
  }
  ss << " |";
  for (int col = 0; col < Board::kBoardWidth; ++col) {
    ss << ' ' << CenterStr(std::to_string(col), Board::kBoardWidth + 1) << '|';
  }
  return ss.str();
}

std::string CenterStr(const std::string& str, int col_width) {
  // quick and easy (but error-prone) implementation
  auto padl = (col_width - str.length()) / 2;
  auto padr = (col_width - str.length()) - padl;
  std::string strf = std::string(padl, ' ') + str + std::string(padr, ' ');
  return strf;
}